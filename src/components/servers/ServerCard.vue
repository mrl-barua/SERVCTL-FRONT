<template>
  <div :class="['server-card', server.env]">
    <div class="card-top">
      ...
      <span :class="['card-env-badge', `badge-${server.env}`]">{{
        envLabels[server.env]
      }}</span>
    </div>
    ...
    <div class="card-status">
      ...
      <span
        v-if="server.uptime > 0"
        style="font-size: 10px; color: var(--text3); margin-left: 8px"
        >uptime {{ server.uptime }}%</span
      >
    </div>
    ...
    <div class="uptime-bar">
      <div
        :class="[
          'uptime-fill',
          server.uptime >= 99 ? '' : server.uptime >= 90 ? 'warn' : 'bad',
        ]"
